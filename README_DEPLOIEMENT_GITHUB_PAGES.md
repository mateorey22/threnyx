...
| Fichier | Rôle |
|---|---|
| `index.html` | Application MeshChat avec bouton d’installation et enregistrement du service worker. |
...
Après le déploiement, ouvrez l’URL HTTPS dans Chrome Android. Ouvrez ensuite le menu `⋮` et choisissez **Installer l’application** — et non **Ajouter à l’écran d’accueil**. Chrome doit afficher la fenêtre d’installation avec l’icône MeshChat. Une fois acceptée, l’icône apparaît parmi les applications et l’ouverture se fait en fenêtre autonome, sans URL Chrome.[1] [2]
...
> Le kit ne prétend pas recevoir des messages lorsque MeshChat est fermé. Un service worker ne peut pas garder une connexion Nostr ouverte en permanence ; il se réveille pour une requête, un événement de synchronisation ou un Push.[4]
